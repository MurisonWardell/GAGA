...
yeastDataSolution=gaga(BYB1_G07_pruned, number_of_clones=6, iterations=3000)
...
Although solutions with 6 clones generally score the best, we found a subtly solution with 7 clones that explains the data even better.
Therefore, the output of that solution is presented below.
